...
# 7. Save all student data to a text file_handling.
# 8. Load student data from a text file_handling.
# 9. Exit the program.
#
# Available Courses:
...
class Student:
    ...
    def __init__(self,s_id, name, year_enrollment):
        self.s_id = s_id # Instance variable
        self.name = name  # Instance variable
        self.year_enrollment = year_enrollment  # Instance variable

    # Object function
    def txt_file_row_str(self):
        return f"{self.s_id},{self.name},{self.year_enrollment}\n"

    # Object function
    def display(self):
        print(f"Student id: {self.s_id}, Name: {self.name}, Year Enrollment: {self.year_enrollment}")
...
def save_student(stu):
    with open("student_record.txt", "a") as file:
        row = stu.txt_file_row_str()
        file.write(row)
...
def load_students_data():
    ...
    with open("student_record.txt", "r") as students:
        for row in students:
            ...
            student_info[s_id] = Student(s_id, name, year_enrollment)
...
def add_student():
    ...
    if s_id in student_info:
        ...
        stu = Student(s_id, name, year_enrollment) # Create a Student object
        student_info[s_id] = stu # store it in the dictionary
        save_student(stu)
...
def search_by_year_enrollment(): #list , year of enrollment
    ...
    result = []
    for stu in student_info.values():
        if y_enrollment in stu.year_enrollment:
            name = stu.name
            s_id = stu.s_id
